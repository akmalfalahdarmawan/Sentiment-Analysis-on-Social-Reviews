...
# ======================= SIDEBAR (versi lebih ramah) =======================
with st.sidebar:
    st.subheader("⚙️ Pengaturan (sederhana)")

    # 1) Profil prediksi yang mudah dipahami
    profil_opsi = {
        "Seimbang (cukup akurat)": "Gunakan saat ingin lebih hati-hati menandai NEGATIF.",
        "Tanggap Keluhan (lebih sensitif)": "Gunakan saat ingin cepat menangkap keluhan (lebih banyak review jadi NEGATIF).",
        "Kustom (bisa diatur sendiri)": "Atur ambang NEGATIF sesuai kebutuhan."
    }

    # Catatan: tidak pakai session_state.set(...) supaya tidak muncul warning
    profil = st.radio(
        "Pilih gaya prediksi",
        list(profil_opsi.keys()),
        index=2,  # default pertama kali: Kustom
        key="profile",
        help="Pilih cara kerja model sesuai kebutuhan operasional."
    )
    st.caption(profil_opsi[profil])

    # 2) Ambang NEGATIF
    # - Untuk "Seimbang" & "Tanggap", pakai nilai dari artefak
    # - Untuk "Kustom", user atur sendiri (default 0.50)
    if profil.startswith("Kustom"):
        thr = st.slider(
            "Seberapa mudah dikategorikan NEGATIF?",
            min_value=0.00, max_value=1.00, value=0.50, step=0.01,
            key="thr_custom",
            help=(
                "Semakin kecil angka, semakin sensitif → lebih banyak review menjadi NEGATIF.\n"
                "Semakin besar angka, lebih konservatif → hanya yang benar-benar negatif."
            )
        )
    else:
        thr = THR_BAL if profil.startswith("Seimbang") else THR_HR
        st.metric(
            label="Ambang NEGATIF (otomatis)",
            value=f"{thr:.2f}",
            help="Nilai bawaan dari model untuk profil ini."
        )

    # Ringkasan singkat yang mudah dimengerti
    st.caption(
        f"Ambang dipakai: **{thr:.2f}** • Urutan kelas: {', '.join(CLASSES)} • "
        f"Seimbang={THR_BAL:.2f} | Tanggap={THR_HR:.2f}"
    )

    # 3) Pengaturan lanjutan (opsional)
    with st.expander("Pengaturan lanjutan (opsional)"):
        low_conf = st.slider(
            "Tandai prediksi 'ragu-ragu'",
            min_value=0.00, max_value=1.00, value=0.45, step=0.01,
            key="low_conf",
            help="Prediksi dengan confidence di bawah nilai ini akan diberi bendera untuk direview manual."
        )
        use_override = st.checkbox(
            "Tangkap kata negatif khas (mis. 'bawel', 'zonk', 'kurang_*')",
            value=True, key="use_override",
            help="Jika frasa negatif kuat terdeteksi, hasil bisa dipaksa NEGATIF kecuali model sangat yakin ke kelas lain."
        )
